...
const TransactionDetail = {
  getAllByTransactionID: (id) => {
    return new Promise((resolve, reject) => {
      connectDB()
        .then((connection) => {
          connection.query("SELECT id AS transaction_detail_id, transaction_id, menu_id, menu_varian_id, serving_type_id, total_item, note_item FROM transaction_details WHERE transaction_id = ?", id, (error, results) => {
            disconnectDB();
            if (error) {
              reject(error);
            } else {
