  ...
  getByCartIdsShift: (cartIdS) => {
    return new Promise((resolve, reject) => {
      connectDB()
        .then((connection) => {
          connection.query(
            "SELECT  cart_details.cart_id, cart_details.id AS cart_detail_id, cart_details.menu_id, cart_details.menu_detail_id, menus.name AS menu_name, menus.menu_type, menu_details.varian, serving_types.name AS serving_type_name, discounts.code AS discount_code, discounted_price, cart_details.price, cart_details.total_price, cart_details.qty FROM cart_details LEFT JOIN discounts ON cart_details.discount_id = discounts.id LEFT JOIN serving_types ON cart_details.serving_type_id = serving_types.id LEFT JOIN menus ON cart_details.menu_id = menus.id LEFT JOIN menu_details ON cart_details.menu_detail_id = menu_details.id WHERE cart_details.cart_id IN (?) AND cart_details.deleted_at IS NULL",
            [cartIdS],
            (error, results) => {
              disconnectDB(connection);
              if (error) {
